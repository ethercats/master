...
catArray.forEach(function(catInArray) {
    ...
    multiplierArray.forEach(function(multiplierValue) {
        for (i = 1; i < 101; i++) {
            ...
            //Set the card_url. The image files must be output first to get an immutable IPFS hash.
            cat.card_url = "https://cards.ethercats.eth.link/" + tokenID + "-" + properties + "-" + findFrame() + ".html"
            //Set the description based on the cat.
            cat.description = "This Chainlink VRF " + rarity.value.toLowerCase() + " " + catInArray + " purrs with a rating of " + cat.attributes[1].value + ", and a multiplier of " + cat.attributes[2].value + "x. It has a total score of " + cat.attributes[3].value + ", and confers the same amount of votes in the EtherCats DAO. The card game properties are " + cat.attributes[4].value + " for North, " + cat.attributes[5].value + " for East, " + cat.attributes[6].value + " for South, and " + cat.attributes[7].value + " for West. " + catInArray + " has the personality suit of " + cat.attributes[8].value.toLowerCase() + ". A rendering of the card can be found at " + cat.card_url + ". Each Founders Series cat is part of NFT history. These fine felines represent the first verifiably random NFTs minted with Chainlink VRF."
            //Set the external_url.
            cat.external_url = "https://nfts.ethercats.eth.link/" + catInArray.toLowerCase() + "/"
            //Set the image urls.
            cat.image = imgBaseUrl + catInArray.toLowerCase() + ".gif"
            cat.card_image = "https://cards.ethercats.eth.link/" + tokenID + ".png"
            //Set the NFT name.
            if (totalScore == 1) {
